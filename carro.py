...
class Carro:
    ...
    def _fator_categoria_pista(self):
        ...
        try:
            diff_cambio = abs(ord(categoria_cambio_carro[0]) - ord(categoria_cambio_ideal.upper()[0]))
            diff_suspensao = abs(ord(categoria_suspensao_carro[0]) - ord(categoria_suspensao_ideal.upper()[0]))
    ...
    def tempo_base(self):
        ...
        if self.engenheiro:
            tempo -= (self.engenheiro.eficiencia_exata * BONUS_DESIGNER_ESCALA * self._fator_influencia("engenheiro"))
    ...
    def tempo_com_variacao(self, desgaste_atual=0.0, combustivel_atual=None):
        if combustivel_atual is None:
            combustivel_atual = self.combustivel_carregado
